package com.example.restcontroller;

import java.util.HashMap;
import java.util.Map;

import org.springframework.security.crypto.bcrypt.BCryptPasswordEncoder;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

import com.example.dto.Member;
import com.example.mapper.MemberMapper;

import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;


@RestController
@RequestMapping(value = "/api/member")
@RequiredArgsConstructor
@Slf4j
public class RestMemberController {

    final JwtUtil jwtUtil; //Component 객체 생성

    final MemberMapper memberMapper;
    final BCryptPasswordEncoder pcpe = new BCryptPasswordEncoder();

/*------------------------------------------------------*/

    //회원가입
    //127.0.0.1:9090/ROOT/api/member/join.json
    //{"id" : "c5", "password" : "c5", "name" : "c5", "age" : "26", "role" : "customer"}
    @PostMapping(value="/join.json")
    public Map<String, Integer> joinPOST(@RequestBody Member member) {
        
        int ret = memberMapper.insertMemberOne(member);

        Map<String, Integer> retMap = new HashMap<>();
        retMap.put("result", ret); // 0 실패, 1 성공
        return retMap;
    }

/*------------------------------------------------------*/

    //로그인 => 로그인 성공시 세션에 정보를 저장하고 다른페이지에서 확인
    //로그인 => 토큰을 발생함. (토큰에는 로그인 사용자 정보 만료시간이 포함되어 있음)

    //로그인
    //127.0.0.1:9090/ROOT/api/member/login.json
    //{"id" : "c5", "password" : "c5"}
    @PostMapping(value="/login.json")
    public Map<String, Object> loginPOST(@RequestBody Member member) {

        log.info("RestMember => {}", member.toString());
        
        //1. 아이디를 이용해서 회원정보 가져오기
        Member retMember = memberMapper.selectMemberOne1(member.getId());
        
        Map<String, Object> retMap = new HashMap<>();
        retMap.put("result", 0); // 0 실패, 1 성공
        
        if(retMember != null){

            //2. 가져온 암호와 전달된 암호가 일치하는지 확인
            if(pcpe.matches(member.getPassword(), retMember.getPassword())){

                //3. 암호가 일치하면 토큰 발행
                retMap.put("result", 1); // 0 실패, 1 성공
                retMap.put("token", jwtUtil.generateToken(member.getId(), "customer"));  //4시간 토큰 발행 (로그인 성공시)
            }

        }
        return retMap; 
        
    }

/*------------------------------------------------------*/


    
    
}
